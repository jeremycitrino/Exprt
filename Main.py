#!/usr/bin/env python
# nostop.py – Optimised REST Polling + Foreground Service (Android adapted)
...
def prevent_sleep():
    # Just keep the Python process alive with dummy threads.
...
def main():
    ...
    save_pid()
    prevent_sleep()
    # Android service already handles wake lock, but we keep prevent_sleep for thread health.
    ...
    # Do NOT open browser on Android
    ...
    atexit.register(lambda: (save_all_state(), save_config(), save_watchlist(), release_wakelock()))
    try:
        while running:
            time.sleep(1)
